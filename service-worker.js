...
const main = (self) => {
    try {
        ...
        const makeBundledAreaAvailableOffline = async (areaName, onProgress = (aun, current, total) => { }, onError = (message) => { }) => {
            ...
            try {
                ...
                await importAreaData(zip, db, cache, onProgress, onError);
                ...
                for (const subareaId of subareaIds) {
                    ...
                    await importAreaData(subareaZip, db, cache, onProgress, onError);
                }
        ...
        class LocalAreaManager {
            ...
            async onWsConnection(player, client) {
            ...
            async onWsMessage(player, client, msg) {
        ...
        const getAreaOrSubareaIdFor = async (player, areaGroupId) => {
            return (await idbKeyval.get(`area-current-subarea-${areaGroupId}`)) || areaGroupId;
        };
        const findSubareaFor = async (areaUrlName, areaGroupId, subareaName) => {
            const db = await dbPromise;
        ...
        class ArchivedAreaManager {
            ...
            isLocked;
            isUnlisted;
            isSinglePlayerExperience;
            explorerChatAllowed;
            mpv;
            constructor(wssUrl, areaId, data, db, possessionsMgr) {
            ...
            async getMinimapData_Tile(x, y) {
                ...
                if (fromDb !== undefined)
                    return { x, y, id: fromDb, pn: placeNames };
            ...
            async setCurrentSubarea(subareaName) {
                    ...
                    switch (parsedMsg.m) {
                        ...
                        case msgTypes.MAP_EDIT: {
                            ...
                            if (def) {
                                ...
                                const res = await this.storageMgr.addPlacement(x, y, { tid, rotation, flip }, {
                        ...
                        case msgTypes.TELEPORT: {
                            ...
                            if (parsedMsg.data === null) {
                                ...
                                const wantedDestination = parsedMsg.data.tol;
        ...
        class AreaManagerManager {
            ...
            async getByWSSUrl(wssUrl) {
                ...
                if (am)
                    ...
                    return await this.makeAreaManager("shouldnthappen_" + String(Date.now()));
            }
            async getByAreaId(areaId) {
                ...
                // @ts-ignore let's pretend the other one doesn't exist, I'll delete it soon
                    ...
                    return await this.makeAreaManager(areaId);
            }
            async getByAreaName(player, areaUrlName) {
        ...
        const makeFakeAPI = async (areaManagerMgr, areaPossessionsMgr) => {
            const db = await dbPromise;
            ...
            router.post("/j/f/gs", async ({ request, json }) => {
                ...
                const writableConfig = {
                    name: "Todo Writable",
                    areaGroupId: "544fe6976e52f57f4d85702b",
            ...
            router.get("/j/n/guc/", async ({ json }) => {
                ...
                if (!lastReadDate)
                    return json(news.length);
                else
                    return json(news.filter(n => n.date > lastReadDate).length);
            });
            // GetLatestNews
            ...
            // #endregion routes
            const handle = async (method, pathname, event, player) => {
        ...
        const handleFetchEvent = async (event) => {
            try {
                ...
                if (url.host === originUrl.host || ["static.manyland.com"].includes(url.hostname)) {
                    ...
                    if (url.pathname.startsWith("/_code/"))
                        return fetch(event.request);
                    // Manyland static assets
                    // TODO: rename this, since there's an area named "static" lol
                    if (url.pathname.startsWith("/static/data/area-thumbnails/"))
                        return cache.getAreaThumbRes(event.request);
                    if (url.pathname.startsWith("/static/data/v2/"))
                        return fetch(event.request);
                    if (url.pathname.startsWith("/static/offlineland/"))
                        return fetch(event.request);
                    if (url.pathname.startsWith("/static/"))
                        return cache.getOrSetFromCache(CACHE_NAME, event.request);
                    ...
                    if (url.pathname === "/media/painter/spritesheet.png")
                        return cache.getOrSetFromCache(CACHE_NAME, new Request(self.origin + "/static/media/painter/spritesheet.png"));
                    if (url.pathname === "/media/painter/cursor_floodFill.png")
                        return cache.getOrSetFromCache(CACHE_NAME, new Request(self.origin + "/static/media/painter/cursor_floodFill.png"));
                    if (url.pathname === "/media/painter/cursor_pickColor.png")
                        return cache.getOrSetFromCache(CACHE_NAME, new Request(self.origin + "/static/media/painter/cursor_pickColor.png"));
                    // Manyland API
        ...
        const handleDataImport = async (file, key, client) => {
            ...
            try {
                ...
                if (zip.file("profile.json")) {
                    try {
                        const profile = await importPlayerData(zip, db, cache, (current, total) => client.postMessage({ m: "IMPORT_PROGRESS", data: { key, current, total } }), (message) => client.postMessage({ m: "GENERIC_ERROR", data: { message } }));
                    ...
                    const data = await importAreaData(zip, db, cache, (areaUrlName, current, total) => client.postMessage({ m: "IMPORT_PROGRESS", data: { key, areaUrlName, current, total } }), (message) => client.postMessage({ m: "GENERIC_ERROR", data: { message } }));
        ...
        const handleDeleteAreaByName = async (areaUrlName, client) => {
            try {
        ...
        const handleLoadArea = async (areaUrlName, client) => {
            try {
                await makeBundledAreaAvailableOffline(areaUrlName, (areaUrlName, current, total) => client.postMessage({ m: "LOAD_AREA_PROGRESS", data: { areaUrlName, current, total } }), (message) => client.postMessage({ m: "GENERIC_ERROR", data: { message } }));
