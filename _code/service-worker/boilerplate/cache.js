...
const getOrSetFromCache = async (cacheName, request) => {
...
const makeCache = (originUrl, SpriteGroundBlob) => {
    ...
    const addToCache = async (cacheName, path, blob) => {
        const cache = await caches.open(cacheName);
        const response = new Response(blob);
        await cache.put(new URL(self.origin + path), response);
    };
    const getAreaNameFromThumbnailReq = (url) => {
        const start = "/static/data/area-thumbnails/".length;
        return url.pathname.slice(start, -4);
    };
    const addAreaThumb = (areaUrlName, thumbnail) => addToCache(CACHE_THUMBS, `/static/data/area-thumbnails/${areaUrlName}.png`, thumbnail);
    const getAreaThumbRes = async (req) => {
